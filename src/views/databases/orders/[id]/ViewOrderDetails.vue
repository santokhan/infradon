...
watchEffect(async () => {
  ...
  async function getOrderById(order_id: string) {
    try {
      ...
      const order: any = await orders_db.get(order_id);

      // Check if the order has a product_id field
      if (order.product_id) {
        // Fetch the product document using the product_id
        const productDoc = await products_db.get(order.product_id);
        order.product = productDoc; // Include product data in the order document
      }

      order.value = order; // Assign the enriched order data
    } catch (error) {
...
<template>
  <div class="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <div class="text-lg text-gray-600">#{{ order?._id }}</div>
    </div>

    <!-- Product Information -->
    <div class="mb-6" v-for="product in order?.product" :key="product._id">
      <div class="flex items-center space-x-4">
        <img src="" :alt="'Product Image'" class="w-24 h-24 object-cover rounded-lg border">
        <div class="flex-1">
          <h2 class="text-lg font-semibold text-gray-800">Product Name</h2>
          <p class="text-gray-600 text-sm">Product Content</p>
        </div>
      </div>
    </div>
